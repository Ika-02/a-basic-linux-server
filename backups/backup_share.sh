...
BACKUP_SOURCE="/share"
BACKUP_DESTINATION="/mnt/backup/share"
LATEST_LINK="/mnt/backup/share/latest"
...
perform_backup() {
    ...
    # Create the backup
    tar --listed-incremental="${SNAPSHOT_FILE}" --exclude="/share/quarantine" -czf "${BACKUP_FILE}" -C "${BACKUP_SOURCE}" .
    if [ $? -ne 0 ]; then
